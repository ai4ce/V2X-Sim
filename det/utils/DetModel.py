...
class PixelwiseWeightedFusionSoftmax(nn.Module):
    def __init__(self, config, layer=3, in_channels=13, shared_img_encoder='unified'):
        super(PixelwiseWeightedFusionSoftmax, self).__init__()
        self.motion_state = config.motion_state
        if config.only_det:
            self.out_seq_len = 1
        else:
