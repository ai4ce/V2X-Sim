...
import torch
from utils.DetModel import PixelwiseWeightedFusionSoftmax, FaFNet
    ...
    def predict(self,data,validation=True):
        ...
        if self.config.flag.startswith('when2com') or self.config.flag.startswith('who2com'):
            if self.config.split == 'train':
                ...
                result = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size, inference=config.inference, training=False)
        else:
            result = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size)
        N = bev_seq.shape[0]
