...
def create_data(config, nusc, current_agent, config_global, scene_begin, scene_end):
    ...
    for scene_idx in res_scenes[scene_begin:scene_end]:
        ...
        while channel_flag == True:
            ...
            all_pc, all_times, trans_matrices, trans_matrices_no_cross_road, target_agent_id, num_sensor = \
            ...
            save_data_dict['trans_matrices_no_cross_road'] = trans_matrices_no_cross_road
            ...
            save_data_dict['all_pc_teacher_no_cross_road'] = all_pc_teacher_no_cross_road.points
            ...
            if adj_seq_cnt == config.num_adj_seqs:
                ...
                print('len save_data_dict_list', len(save_data_dict_list))
...
def convert_to_dense_bev(seq_data_dict, config):
    ...
    for i in range(num_sweeps):
        res_teacher, voxel_indices_teacher_no_cross_road = voxelize_occupy(pc_list[i], voxel_size=config.voxel_size,
                                                             extents=config.area_extents, return_indices=True)
        voxel_indices_list_teacher_no_cross_road.append(voxel_indices_teacher_no_cross_road)
    ...
    if config.split == 'val' or config.split == 'test':
        ...
        return voxel_indices_list, voxel_indices_list_teacher, voxel_indices_list_teacher_no_cross_road, padded_voxel_points, trans_matrices_warping, trans_matrices_no_cross_road, \
               label, reg_target, allocation_map, gt_max_iou, reg_loss_mask, motion_state, visibility_maps, target_agent_id, num_sensor
...
def convert_to_sparse_bev(config, dense_bev_data, use_motion_state=False):
    if config.split == 'val' or config.split == 'test':
        ...
        save_voxel_indices_list, voxel_indices_list_teacher, voxel_indices_list_teacher_no_cross_road, save_voxel_points, save_trans_matrices_warp, save_trans_matrices_no_cross_road, \
        save_label, save_reg_target, save_allocation_mask, gt_max_iou, reg_loss_mask, motion_state, visibility_maps, target_agent_id, num_sensor \
            = dense_bev_data
    ...
    if config.split == 'train':
        ...
        save_data_dict['voxel_indices_teacher_no_cross_road'] = voxel_indices_list_teacher_no_cross_road[0].astype(np.int32)
...
if __name__ == "__main__":
    ...
    for current_agent in range(args.from_agent, args.to_agent):
        ...
        savepath += args.split
        check_folder(savepath)
        savepath += '/agent' + str(current_agent)
        check_folder(savepath)
