...
def main(args):
    ...
    config.flag =  flag
    config.split = 'test'
    valset = CarscenesDataset(dataset_roots=[f'{args.data}/agent{i}' for i in range(5)], config=config, config_global=config_global, split='val', val=True)
    ...
    if flag == 'upperbound' or flag.startswith('lowerbound'):
        ...
        model = V2VNet(config, gnn_iter_times=1, layer=args.layer, layer_channel=256)
