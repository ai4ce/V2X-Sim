    ...
    def pick_single_agent(self, agent_id, idx):
        ...
        if idx in self.cache[agent_id]:
            ...
            if gt_data_handle == 0:
               ...
               if self.val:
                  return None, None, label_one_hot,\
                  reg_target, reg_loss_mask, anchors_map, vis_maps, [{"gt_box":[[0, 0, 0, 0], [0, 0, 0, 0]]}], [seq_file],\
                         0, 0, np.zeros((5,4,4))
               else:
                  return None, None, label_one_hot, reg_target, reg_loss_mask, anchors_map, vis_maps, 0, 0, np.zeros((5,4,4))
            else:
               gt_dict = gt_data_handle.item()
        ...
        #    padded_voxel_points_teacher = padded_voxel_points_teacher.astype(np.float32)
           ...
           if self.val:
               return None, None, label_one_hot, reg_target, reg_loss_mask, anchors_map, vis_maps, [{"gt_box":gt_max_iou}], [seq_file], \
                       None, None, None


           else:
               return None, None, label_one_hot, reg_target, reg_loss_mask, anchors_map, vis_maps,\
                      None, None, None
...
if __name__ == "__main__":
    ...
    data_carscenes = CarscenesDataset(dataset_roots=['/data_1/yml/disconet/dataset/test/agent0'], split = split, config=config, config_global=config_global, val=True)
