...
def create_data(config,nusc,current_agent,config_global,scene_begin,scene_end, data_root):
    ...
    valdataset = CarscenesDataset(dataset_roots=[f'{data_root}/agent{i}' for i in range(args.num_agent)], config=config, config_global=config_global, split='val', val=True)
    file2id = {}
