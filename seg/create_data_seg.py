...
from data.data_util import voxelize_occupy
from data.obj_util import *
from nuscenes.utils.data_classes import LidarPointCloud
from utils.obj_class_config import classes_remap
...
def create_data(config, nusc, current_agent, config_global, scene_begin, scene_end):
    ...
    for scene_idx in res_scenes[scene_begin:scene_end]:
        ...
        while channel_flag == True:
            ...
            all_pc_single_view, all_times, trans_matrices, trans_matrices_no_cross_road, target_agent_id, num_sensor = LidarPointCloud.from_file_multisweep_warp2com_sample_data(current_agent, nusc, curr_sample_data, return_trans_matrix=True)
            all_pc_teacher,_  = LidarPointCloud.from_file_multisweep_upperbound_sample_data(nusc, curr_sample_data, return_trans_matrix=False)
            all_pc_teacher_no_cross_road, _ = LidarPointCloud.from_file_multisweep_upperbound_sample_data(nusc, curr_sample_data, return_trans_matrix=False, no_cross_road=True)
            ...
            bev_pix = cv2.resize(bev_pix, dsize=(256, 256), interpolation=cv2.INTER_NEAREST)
            # plt.imshow(bev_pix)
            # plt.show()
            ...
            seq_idx = 0
            dense_bev_data_single = convert_to_dense_bev(save_data_dict, config, 'pc_single')
            dense_bev_data_teacher = convert_to_dense_bev(save_data_dict, config, 'pc_teacher')
            dense_bev_data_teacher_no_cross_road = convert_to_dense_bev(save_data_dict, config, 'pc_teacher_no_cross_road')

            sparse_bev_data = convert_to_sparse_bev(config, dense_bev_data_single)
            convert_to_sparse_bev_teacher(config, dense_bev_data_teacher, sparse_bev_data, 'voxel_indices_teacher')
            convert_to_sparse_bev_teacher(config, dense_bev_data_teacher_no_cross_road, sparse_bev_data, 'voxel_indices_teacher_no_cross_road')

            sparse_bev_data['trans_matrices'] = trans_matrices
            sparse_bev_data['trans_matrices_no_cross_road'] = trans_matrices_no_cross_road
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--root', default='/home/dekunma/CS/ai4ce/V2X-Sim-2', type=str, help='Root path to nuScenes dataset')
    parser.add_argument('-s', '--split', default='train', type=str, help='The data split [train/val/test]')
    parser.add_argument('-b', '--scene_begin', default=0, type=int, help='Index of begining scene')
    parser.add_argument('-e', '--scene_end', default=1, type=int, help='Index of end scene + 1')
    parser.add_argument('-p', '--savepath', default='/home/dekunma/CS/ai4ce/V2X-seg-2', type=str, help='Directory for saving the generated data')
    parser.add_argument('--from_agent', default=0, type=int, help='start from which agent')
    parser.add_argument('--to_agent', default=6, type=int, help='until which agent (index + 1)')
