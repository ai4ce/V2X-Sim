...
#            current_sd_rec = nusc.get('sample_data', current_sd_rec['next'])
    ...
    def from_file_multisweep_upperbound_sample_data(cls,
        ...
        # cross road has 84 sensors; there are 30 types of sensors in total; we plus 1 because the following loop is 0-indexed
        num_sensor = ((len(sample_record['data']) - 84) // 15) + 1
        ...
        for k in range(num_sensor):
            ...
            trans_matrix_aug = transform_matrix([0,0,0], Quaternion([0.7071, 0, 0, 0.7071]), inverse=False)
            # trans_matrix_aug = transform_matrix([0, 0, 0], Quaternion([0.9239, 0, 0, 0.3827]), inverse=False)
            current_pc.transform(trans_matrix_aug)
    ...
    def get_instance_boxes_multisweep_sample_data(cls,
        ...
        for box in boxes:
            if box is not None:
                ...
                y0 = box.center[1]
                z0 = box.center[2]
                box.translate(-np.array([x0, y0, z0]))
                box.rotate(Quaternion([0.7071, 0, 0, 0.7071]))
                box.translate(np.array([y0, -x0, z0]))
